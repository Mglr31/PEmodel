function result=ParabolicEquationParra(simu,source,varargin)
%result=ParabolicEquationParra(simu,source)
%A function that apply the Parabolic Equation model to return a field of SPL in the (z,r) plan.
%Same as ParabolicEquation but if several simulation need to be computed(case of turbulence) the computation is done in parrallel 
% Inputs:
...
%
%
%       'Averaged':     default 0, if equal to 1 the atmospherique profile will
%                       be averaged to suppress the effect of sound refraction
%
%       "Light":    default0, if equal to one mu fields wont be saved and
...
psigrid=kgrid;
psigrid(:)=0;
ka=kgrid(m,1);% k at the ground(or maybe at a medium altitude??)
k0=kgrid(m,1);% k at the ground(m=ground)
...
if isempty(turbulence)%simulation without atm turbulence
    ...
    parfor k=1:N
        psigridloc=psigrid;%local psi grid for the parrallel computation
        ...
        for j=2:n-1     %propagating the sound field for one random realisation
            ...
            psigridloc(:,j)=flipud(tridiagonal(M2,M1*flipud(psigridloc(:,j-1))).*exp(1i*ka*mu2*simu.Rstep));

        end
        if light
            allSPL{k}=single(10*log10(0.5*abs(psigridloc.*psi2p_mat).^2/pref^2));%grid of the sound pressure level;
        else
            allSPL{k}=10*log10(0.5*abs(psigridloc.*psi2p_mat).^2/pref^2);%grid of the sound pressure level;
...
[~,indr]=min(abs(rvect-0.1));
[~,indz]=min(abs(zvect-source.Zsource));
...
%%%
result.SPLmin=result.SPL(indz,end);
